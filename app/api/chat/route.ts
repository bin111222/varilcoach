...
- Timezone: ${timezone}
- Current date/time (human): ${nowHuman}
- Current date/time (ISO): ${nowIso}
...
- For questions using relative time (today, tomorrow, this week), use the TIME CONTEXT above as the source of truth.`;
...
export async function POST(req: Request) {
  try {
    ...
    const currentWeek = settings?.currentWeek ?? 1;
    const weekData = await Week.findOne({ number: currentWeek }).lean();
    const recentProgress = await Progress.find()
      .sort({ date: -1 })
      .limit(10)
      .lean();

    const athleteName = settings?.athleteName ?? "Athlete";
    const goals = settings?.goals ?? [];
    const injuries = settings?.injuries ?? "";
    const model = settings?.openaiModel ?? "gpt-4o";
          ...
          .map(
            (d) =>
              `${d.label} (${d.name}): ${
                d.exercises?.length
                  ? d.exercises
                      .map((e) => `${e.name} ${e.sets} @ ${e.load}`)
                      .join(", ")
                  : d.type === "swim"
                  ? `${d.drills?.length ?? 0} drills`
                  : d.type === "run"
                  ? d.runStats?.map((r) => `${r.label}: ${r.value}`).join(", ")
                  : d.type === "mma"
                  ? "MMA Session"
                  : "Rest"
              }`
